...
def calculate_quality_indicators(construction_instructions, parameters, fjs, cell_mat_m, symm_mats_vecs, index_vec_h, intensities, stderr):
    ...
    wr2_strong  = np.sqrt(np.sum(1/stderr[i_over_2sigma]**2 * (intensities[i_over_2sigma] - parameters[0] *  np.abs(structure_factors[i_over_2sigma])**2)**2) / np.sum(1/stderr[i_over_2sigma]**2 * intensities[i_over_2sigma]**2))
