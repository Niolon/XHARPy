...
def calc_f0j(cell_mat_m, element_symbols, xyz, uij, index_vec_h, symm_mats_vecs, options_dict=None, restart=None, save='gpaw.gpw', explicit_core=True):
    ...
    atom2_indexes = np.where([symbol == 'H' for symbol in element_symbols])[0]

    cart = np.einsum('xy, zy -> zx', cell_mat_m, xyz)
    distances = np.linalg.norm(cart[atom2_indexes, None, :] - cart[None, :], axis=-1)
    distances[distances < 1e-3] = 9999
    atom1_indexes = np.argmin(distances, axis=1)

    cell_mat_g = np.einsum('ja, jb -> ab', cell_mat_m, cell_mat_m)
    cell_mat_f = np.linalg.inv(cell_mat_m).T
    lengths_star = np.linalg.norm(cell_mat_f, axis=0)
    u_mats = uij[:, np.array([[0, 5, 4],
                            [5, 1, 3],
                            [4, 3, 2]])]
    betas = u_mats * np.outer(lengths_star, lengths_star)

    xyz1s = xyz[atom1_indexes]
    xyz2s = xyz[atom2_indexes]

    d0s = np.einsum('xy, zy -> zx', cell_mat_m, xyz2s - xyz1s)

    x0s = d0s / np.linalg.norm(d0s, axis=-1)[:,None]

    xgx = np.einsum('ax, xy, ay -> a', x0s, cell_mat_g, x0s)
    xg = np.einsum('ax, xy -> ay', x0s, cell_mat_g)
    gx = np.einsum('xy, ay -> ax', cell_mat_g, x0s)

    bg1 = np.trace(betas[atom1_indexes] * cell_mat_g, axis1=-1, axis2=-2)
    xgbgx1 = np.einsum('ax, axy, ay -> a', xg, betas[atom1_indexes], gx)
    av_w1sq = 1 / (2 * np.pi**2) * (bg1 - xgbgx1 / xgx)

    bg2 = np.trace(betas[atom2_indexes] * cell_mat_g, axis1=-1, axis2=-2)
    xgbgx2 = np.einsum('ax, axy, ay -> a', xg, betas[atom2_indexes], gx)
    av_w2sq = 1 / (2 * np.pi**2) * (bg2 - xgbgx2 / xgx)
    ...
    symm_xyz, *_ = expand_symm_unique(element_symbols,
                                      np.array(xyz),
                                      np.array(cell_mat_m),
