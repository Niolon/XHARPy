...
def merge_hkl_df(hkl_df: pd.DataFrame, 
              cell: np.ndarray, 
              symm_strings: List[str]) -> pd.DataFrame:
    """
    Merge equivalent reflections in HKL data using crystallographic symmetry for use
    in refinement (do not refine on unmerged data) by using the functions from cctbx.
    
    Parameters
    ----------
    hkl_df : pd.DataFrame
        DataFrame containing HKL data with at least the following columns:
        'h', 'k', 'l' : Miller indices
        'intensity' : Observed intensities
        'esd_int' : Estimated standard deviations of intensities
    
    cell : np.ndarray
        Unit cell parameters (a, b, c, alpha, beta, gamma) in Angstroms and degrees.
    
    symm_strings : list of str
        List of symmetry operations as strings, e.g., ['x,y,z', '-x,y,-z'].
        These define the space group operations.
    
    Returns
    -------
    pd.DataFrame
        DataFrame containing the merged HKL data with columns:
        'h', 'k', 'l' : Miller indices
        'intensity' : Merged intensities
        'esd_int' : Estimated standard deviations of merged intensities
        'multiplicity' : Number of reflections merged for each unique reflection (for reference, not used)
    """
    ...
    for symm_op in symm_strings:
        try:
            space_group.expand_smx(symm_op)
        except Exception as e:
            print(f"Warning: Could not parse symmetry operation '{symm_op}': {e}")
    ...
    hkl_flex = flex.miller_index([(int(h), int(k), int(l)) for h, k, l in 
                                  zip(hkl_df['h'], hkl_df['k'], hkl_df['l'])])
    intensity_flex = flex.double(hkl_df['intensity'].values)
    sigma_flex = flex.double(hkl_df['esd_int'].values)
    
    miller_set = miller.set(crystal_symmetry, hkl_flex, anomalous_flag=True)
    
    intensity_array = miller_set.array(intensity_flex, sigma_flex).set_observation_type_xray_intensity()
    
    intensity_array = intensity_array.eliminate_sys_absent()
    
    merged = intensity_array.merge_equivalents()
    merged_array = merged.array()
    
    print("Merging reflections using cctbx.")
    print("R-factor:", merged.r_linear())
    print("Number of reflections before merging:", intensity_array.size())
    print("Number of reflections after merging:", merged_array.size())
    
    merged_indices = merged_array.indices()
    merged_data = merged_array.data()
    merged_sigmas = merged_array.sigmas()
    
    merged_df = pd.DataFrame({
        'h': [h[0] for h in merged_indices],
        'k': [h[1] for h in merged_indices],
        'l': [h[2] for h in merged_indices],
        'intensity': list(merged_data),
        'esd_int': list(merged_sigmas),
        'multiplicity': list(merged.redundancies().data())
    })
    return merged_df
