...
def resolve_restraints(xyz, uij, restraints, cell_mat_m):
    ...
    for restraint in restraints:
        if type(restraint).__name__ == 'FixedDistanceRestrInd':
            ...
            return_sum += (np.linalg.norm(diff) - restraint.distance)**2 / restraint.esd**2
            ...
            return_sum += np.sum((distances - distances.mean)**2) / restraint.esd**2
        elif type(restraint).__name__ == 'SameADPRestrInd':
            return_sum += np.sum((uij[restraint.atom1_index] - restraint.mult * uij[restraint.atom2_index])**2) / restraint.esd**2
            ...
            return_sum += (diff_u[2, 2]**2 + diff_u[1, 2]**2 + diff_u[0, 2]**2) / restraint.esd**2
        else:
